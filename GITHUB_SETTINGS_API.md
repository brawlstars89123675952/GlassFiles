...
### Watched repositories / subscriptions

**Статус:** ✅ Доступно
**Приоритет:** 🎯 MED

- `GET /user/subscriptions` — репозитории, за которыми следит аккаунт
- `GET /repos/{owner}/{repo}/subscription` — проверить watch/subscription
- `PUT /repos/{owner}/{repo}/subscription` — включить watch
- `DELETE /repos/{owner}/{repo}/subscription` — убрать watch

### Repository invitations for current user

**Статус:** ✅ Доступно
**Приоритет:** 🎯 MED

- `GET /user/repository_invitations` — входящие приглашения в репозитории
- `PATCH /user/repository_invitations/{invitation_id}` — принять приглашение
- `DELETE /user/repository_invitations/{invitation_id}` — отклонить приглашение
