...
### UI optimization / anti-neuroslop phase
- Следующий этап пока что переключается с расширения API coverage на оптимизацию и ручную доработку UI.
- Цель этапа:
  - убрать ощущение "нейрослопа" в большинстве окон
  - привести экраны к более цельному, спокойному и прикладному виду
  - уменьшить визуальный шум, случайные карточки, перегруженные кнопки и однотипные generated-looking блоки
  - выровнять spacing, typography, action placement, empty/loading/error states
  - сделать основные workflows быстрее читаемыми и менее похожими на набор автоматически добавленных секций
- Приоритетные зоны:
  - GitHub repo detail tabs
  - PR/detail/diff/review screens
  - Projects, Security, Webhooks, Notifications modules
  - markdown/README rendering and long-text presentation
- Правило на время этапа:
  - новые GitHub API endpoints добавлять только если они нужны для конкретного UX fix
  - сначала чистить существующие окна и поведение
  - каждый проход делать по одному экрану или одному небольшому flow, чтобы не смешивать изменения
- Проверка:
  - локальную Android compile-проверку не запускать без отдельной просьбы пользователя
